...
<el-card class="item-padding">
  ...
  <el-row :gutter="20">
    <el-col :offset="3" :span="21" class="top-line">
      ...
      <el-button class="add-rules" @click="addCdnAdvanceCacheRules">新增</el-button>
    </el-col>
  </el-row>
  <el-row :gutter="20">
    <el-col :offset="3" :span="21" class="table-content" width="120" v-show="selectCacheType === 'yes'">
      <el-table :data="cdnAdvanceCache.cacheRules" border style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="操作" width="90" align="right" class-name="row-padding-right" lable-class-name="row-padding-right">
          <template scope="scope">
              <el-button v-if="scope.row.isEditing" type="text" size="small" @click="handleCdnCacheRuleSave(scope)">保存</el-button>
              <el-button v-else type="text" @click="handleEditRule(scope)">编辑</el-button>
              <el-button v-if="scope.row.isEditing" type="text" size="small" @click="handleCancelEdit(scope)">取消</el-button>
              <el-popover ref="ifDelCacheItem" placement="top" width="100" trigger="click" v-model="popoverCacheArray[scope.$index].flag">
                <p>确认删除当前行吗？</p>
                <div style="text-align: right; margin: 0">
                  <el-button size="mini" type="text" @click="handleCancelDel(scope.$index)">取消</el-button>
                  <el-button type="primary" size="mini" @click="handleConfDel(scope.$index,scope.row)">确定</el-button>
                </div>
              </el-popover>
              <el-button v-show="!scope.row.isEditing" type="text" v-popover:ifDelCacheItem>删除</el-button>
            </template>
        </el-table-column>
      </el-table>
    </el-col>
      ...
      <el-table :data="cdnAdvanceCache.noCacheRules" border style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="操作" width="90" align="right" class-name="row-padding-right" lable-class-name="row-padding-right">
            <template scope="scope">
              ...
              <el-button v-show="!scope.row.isEditing" type="text" v-popover:ifDelNoCacheItem>删除</el-button>
            </template>
        </el-table-column>
      </el-table>
    </el-col>
