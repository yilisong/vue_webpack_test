...
export default {
  ...
  methods: {
    ...
    getCcAttackChart(params) {
      TaichiResource.getCcAttackChart(params).then(response => {
        if (response.data && (response.data.status.code === 1) && response.data.data) {
          ...
          this.ccAttackChart.unit = response.data.data.trend.y_data.unit
        } else {
    ...
    consoleTimeValue(startTimevalue, endTimevalue) {
      ...
      this.getCcAttackChart(params)
    },
    getCCtopTable() {
      TaichiResource.getCcAttackData().then(response => {
    ...
    nationalDistribution() {
      ...
      this.nationalDistributionChart.setOption({
        ...
        visualMap: {
          min: 0,
          max: this.nationalDistributionSeries[0].value ? this.nationalDistributionSeries[0].value : 10,
          left: 'left',
          top: 'bottom',
    ...
    getNationalData() {
      TaichiResource.getCountryChart().then(response => {
        if (response.data && (response.data.status.code === 1) && response.data.data) {
    ...
    getGlobalData() {
      TaichiResource.getWorldChart().then(response => {
       if (response.data && (response.data.status.code === 1) && response.data.data) {
          this.globalTip = []
          this.globalTop5 = []
