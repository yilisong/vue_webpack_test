  ...
  <div class="cc-statistics">
    ...
    <div class="tableRight">
      ...
      <el-table :data="topFiveIP" v-loading="top5TableLoading" element-loading-text="拼命加载中">
        ...
        <el-table-column prop="loaction.province" label="归属地">
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
        ...
        if (response.data && response.data.data && response.data.data.list) {
          ...
          if (list.length > 0) {
            ...
            if (this.value === '') {
              ...
              this.test = '0'
              this.test2 = '0'
              this.getPlusWebCCTop(params)
              this.getPackageDomainList(params)
              this.changeDomain()
              this.getWebCCGlobalData(params)
              this.getWebCCNationalData(params)
            }
          } else {
    ...
    getPackageDomainList(params) {
      TaichiPlusResource.getPackageDomainList(params).then(response => {
    ...
    getWebCcAttackChart(params) {
      ...
      TaichiPlusResource.getWebCcAttackChart(params).then(response => {
        this.ccAttackChartLoading = false
    ...
    consoleTimeValue(startTimevalue, endTimevalue) {
      ...
      if (this.test2 === '0') {
        ...
        this.getWebCcAttackChart(params)
      }
    },
    getPlusWebCCTop(params) {
      this.top5TableLoading = true
      TaichiPlusResource.getPlusWebCCTop(params).then(response => {
        this.top5TableLoading = false
        if (response.data && response.data.data && (response.data.status.code === 1)) {
          const dateTop = response.data.data
          if (dateTop.top_request_url) {
            const lenUrl = dateTop.top_request_url.length
            if (lenUrl > 4) {
              this.topFiveURL = dateTop.top_request_url.slice(0, 5)
            } else if (lenUrl > 0 && lenUrl < 5) {
              this.topFiveURL = dateTop.top_request_url.slice(0, lenUrl)
            } else {
              this.topFiveURL = []
            }
          } else {
    ...
    getWebCCNationalData(params) {
      ...
      TaichiPlusResource.getWebCCNationalData(params).then(response => {
        this.nationalLoading = false
    ...
    getWebCCGlobalData(params) {
      TaichiPlusResource.getWebCCGlobalData(params).then(response => {
