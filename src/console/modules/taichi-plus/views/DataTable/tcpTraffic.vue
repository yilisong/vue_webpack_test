...
export default {
  ...
  methods: {
    ...
    getPlusTCpFlaw(params) {
      TaichiPlusResource.getPlusTCpFlaw(params).then(response => {
        this.ddosLoading = false
        if (response.data.status.code = 1 && response.data.data.trend) {
          ...
          this.ddosAttackChart.unit = response.data.data.trend.y_data.unit ? response.data.data.trend.y_data.unit : 'KB'
        } else {
          this.ddosAttackChart.unit = 'KB'
